...
object Async extends AsyncInterface {
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    def retry(f: () => Future[A], maxAttempts: Int, attempts: Int): Future[A] = {
      if (maxAttempts == (attempts + 1)) makeAsyncComputation()
      else f().recoverWith {case _: Throwable => retry(f, maxAttempts, attempts + 1)}
    }
